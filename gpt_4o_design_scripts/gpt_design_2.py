...
for i in range(outlet_count):
    angle = i * angle_step
    x = manifold_radius * 2 + outlet_length
    y = 0
    z = (i - (outlet_count - 1) / 2) * outlet_spacing
    outlet = Part.makeCylinder(outlet_radius, outlet_length)
    outlet.rotate(App.Vector(0, 0, 0), App.Vector(0, 1, 0), 90)
    outlet.translate(App.Vector(manifold_radius + outlet_length, 0, z))
